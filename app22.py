...
class AgentState(TypedDict):
    task: str
    loop_count: Annotated[int, operator.add]  # Automatically increments
    manager_plan: str
    ...
    final_output: str
    next_agent: str  # Dynamic routing decision
    conversation_history: list[dict]
    agent_messages: list  # For tool usage tracking
    pushover_sent: bool  # Track if notification was sent
...
# ========== PUSHOVER FUNCTION ==========
def send_pushover_notification(message: str, title: str = "Agent System Output") -> dict:
...
Create a final output (4-6 sentences). Be concise, actionable, and professional."""
    ...
    state["next_agent"] = "pushover"  # Changed to go to pushover
...
def pushover_agent(state: AgentState) -> AgentState:
    """Send final output via Pushover notification"""
    state = add_to_conversation(state, "📱 Pushover", "Sending notification to your phone...", "pushover")
    
    final_output = state.get("final_output", "No output generated")
    task = state.get("task", "Unknown task")
    
    # Create notification message
    notification_title = "Agent System Complete"
    notification_message = f"Task: {task[:100]}...\n\n{final_output[:900]}"
    
    # Send notification
    result = send_pushover_notification(notification_message, notification_title)
    
    if result["success"]:
        message = f"✅ Notification sent successfully to your phone!\n\nMessage preview:\n{notification_message[:200]}..."
        state = add_to_conversation(state, "📱 Pushover", message, "pushover")
        state["pushover_sent"] = True
    else:
        message = f"❌ Failed to send notification: {result['error']}"
        state = add_to_conversation(state, "📱 Pushover", message, "pushover")
        state["pushover_sent"] = False
...
# ========== DYNAMIC ROUTING ==========
def route_next(state: AgentState) -> Literal["ideator", "stylist", "analyst", "editor", "finalizer", "pushover", "end"]:
...
def create_workflow():
    ...
    workflow.add_node("pushover", pushover_agent)
...
def main():
    ...
    st.title("🤖 Enhanced Multi-Agent System with Pushover")
    st.markdown("**Features:** Tool Usage | Dynamic Routing | Real-time Streaming | Max 2 Loops | 📱 Phone Notifications")
    ...
    with st.sidebar:
        ...
        if pushover_token and pushover_user:
            ...
            st.warning("⚠️ Pushover not configured")
            with st.expander("📱 Setup Pushover"):
                st.markdown("""
                1. Sign up at [pushover.net](https://pushover.net)
                2. Install Pushover app on your phone
                3. Create an application at [pushover.net/apps](https://pushover.net/apps)
                4. Add to `.env` file:
                   ```
    ...
    if run_button:
        ...
        if final_state.values.get("final_output"):
            ...
            with col3:
                if final_state.values.get("pushover_sent"):
                    st.success("📱 Sent to Phone")
                else:
                    st.warning("📱 Not Sent")
